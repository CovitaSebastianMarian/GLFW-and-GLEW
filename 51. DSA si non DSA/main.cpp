...
#include "Engine/ShapeTexture2D.h"
#include "2D.hpp"
...
int main() {
	...
	const bool use_dsa = true;
	ShapeShader2D sh;

	ShapeTexture2D t(use_dsa);
	t.load_texture("Images/concreate.jpg", GL_RGB);

	Form2D f(use_dsa, 36);
	f.set_orthographic(0, WindowWidth, WindowHeight, 0);
	f.set_position({ WindowWidth / 2, WindowHeight / 2 });
	f.set_scale({ 300, 300 });
