...
bool KEY_W = false, KEY_S = false, KEY_D = false, KEY_A = false, KEY_SPACE = false;
bool** Map;
Object* player;
Object* mapa;
ObjFile mapafile;
...
float mapsize = 100 * scaleval;
float speed = 1 * scaleval;
float playersize = 10 * scaleval;
Loader<bool> l;

bool InMap(float zpos, float xpos) {
    if (xpos >= -mapsize && xpos <= mapsize && zpos >= -mapsize && zpos <= mapsize) {
        return true;
    }
    return false;
}
...
bool ColisionAround(int zz, int xx) {
    //return true if colisions, return false if doesn't
    int x2, z2;
    float xpos, zpos;
    bool ok = false;
    for (int i = 0; i < 4; ++i) {
        ...
        case 0:
            ...
            break;
        case 1:
            zpos += playersize / 2 - 0.01;
            break;
        case 2:
            xpos -= playersize / 2;
            break;
        case 3:
            xpos += playersize / 2 - 0.01;
            break;
        }
        z2 = (zpos + mapsize) / playersize;
        x2 = (xpos + mapsize) / playersize;

        if (Map[z2][x2] > Map[zz][xx]) ok = true;
    }
    if (ok) return true;
    return false;
}
...
void Config() {
    int xx = x, zz = z;
    if (KEY_A) {
        float xpos = playerxpos - speed - playersize / 2;
        xx = (xpos + mapsize) / playersize;
        if (InMap(playerzpos, xpos) && !ColisionAround(zz, xx)) {
            if (Map[z][xx] <= playerypos) {
                playerxpos = playerxpos - speed;
                xx = (playerxpos + mapsize) / playersize;
                x = xx;
            }
            else {
                playerxpos = (xx + 1.5) * playersize - mapsize;
            }
        }
    }
    if (KEY_W) {
        float zpos = playerzpos - speed - playersize / 2;
        zz = (zpos + mapsize) / playersize;
        if (InMap(zpos, playerxpos) && !ColisionAround(zz, xx)) {
            if (Map[zz][x] <= playerypos) {
                playerzpos = playerzpos - speed;
                zz = (playerzpos + mapsize) / playersize;
                z = zz;
            }
            else {
                playerzpos = (zz + 1.5) * playersize - mapsize;
            }
        }
    }
    if (KEY_S) {
        ...
        if (InMap(zpos, playerxpos) && !ColisionAround(zz, xx)) {
            if (Map[zz][x] <= playerypos) {
                ...
                playerzpos = (zz - 0.5) * playersize - mapsize;
            }
        }
    }
    if (KEY_D) {
        ...
        if (InMap(playerzpos, xpos) && !ColisionAround(zz, xx)) {
            if (Map[z][xx] <= playerypos) {
                ...
                playerxpos = (xx - 0.5) * playersize - mapsize;
            }
        }
    }
    if (KEY_SPACE) {
...
int main() {
    ...
    for (int i = 0; i < 20; ++i) {
        for (int j = 0; j < 20; ++j) {
            if (i == z && j == x) {
                playerxpos = (j + 0.5) * playersize - mapsize;
                playerzpos = (i + 0.5) * playersize - mapsize;
            }
        }
    }
    ...
    for (int i = 0; i < mapafile.KitVector.size(); ++i) {
        ...
        mapa[i].Ortho(-wwindowsize / 2, wwindowsize / 2, -wwindowsize/ 2, wwindowsize / 2, wwindowsize / 2, -wwindowsize / 2);
        mapa[i].Perspective(glm::radians(85.0), 1, 0.01, 100);
    ...
    for (int i = 0; i < playerfile.KitVector.size(); ++i) {
        ...
        player[i].Ortho(-wwindowsize / 2, wwindowsize / 2, -wwindowsize / 2, wwindowsize / 2, wwindowsize / 2, -wwindowsize / 2);
        player[i].Perspective(glm::radians(85.0), 1, 0.01, 100);
    ...
    l.Load("mapa.txt", Map);
    /*
    for (int i = 0; i < l.y_size; ++i, cout<<"\n") {
        for (int j = 0; j < l.x_size; ++j) {
            cout << Map[i][j] << " ";
        }
    }
    */
